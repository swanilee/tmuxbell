...
**사이드바**
- 각 세션 행에 상태 점, 세션 이름, 윈도우 수가 표시됩니다.
- 이름을 클릭하면 메인 영역에서 그 세션이 열립니다. 행 아래로 세션의
  윈도우 목록이 펼쳐져 있고, 윈도우 이름을 클릭하면 바로 그 윈도우로 이동.
- 세션 이름이나 윈도우 이름을 **더블클릭**하면 즉시 편집 가능
  (Enter 저장, Esc 취소).
- 행에 마우스를 올리면 **×** 버튼이 보이고, 세션이나 윈도우를 종료할 수 있습니다.

**메인 영역**
- 상단 탭에 현재 세션의 모든 윈도우가 나열됩니다. 탭 클릭으로 활성 윈도우 전환.
- **+ 새 윈도우** 모달: 이름(선택), 실행 명령(기본 `claude`),
  현재 윈도우의 작업 디렉토리(CWD) 이어받기 체크박스.
- **+ 새 세션** 모달: 이름(선택), 작업 디렉토리(선택) — 그 디렉토리에서 `claude` 실행.

**편의 컨트롤**
- 상단바 좌측의 **‹** 버튼으로 사이드바를 접어 터미널을 거의 전체 너비로
  사용할 수 있습니다. 다시 클릭하면 펼침. 상태 자동 기억.
- **마우스 휠 스크롤백** 토글 (사이드바 하단) — 켜면 어느 pane에서든 휠을
